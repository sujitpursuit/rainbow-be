...
def get_api_response(question, account_number):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {'error': f"Status code returned: {response.status_code}"}, response_time
...
for question in questions:
    response, response_time = get_api_response(question, account_number)
    ...
    if isinstance(response, dict) and 'response' in response:
        ...
        if isinstance(api_response, dict):
            ...
            try:
                ...
                results.append({
                    ...
                    "response_time": str(response_time).split('.')[0] if response_time else 'N/A'
                })
        ...
        results.append({
            ...
            "join_clause": "N/A",
            "table_aliases": "N/A",
            "validation_messages": "Error in response",
            "response_time": str(response_time).split('.')[0] if response_time else 'N/A'
        })
    time.sleep(10)
...
headers = ["question", "sql_query", "data", "summary", "status_code", "where_count",
    "or_clause", "join_status", "join_clause", "table_aliases",
    "validation_messages", "response_time"]
sheet.append(headers)

for result in results:
    sheet.append([
        result["question"], result["sql_query"], result["data"], result["summary"],
        result["status_code"], result["where_count"], result["or_clause"],
        result["join_status"], result["join_clause"], result["table_aliases"],
        result["validation_messages"], result["response_time"]
    ])

wb.save('Insurance.xlsx')
print("Results saved in Insurance.xlsx")
